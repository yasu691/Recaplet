...
## 16:56 Azure OpenAI への切り替え (全件処理対応)

- 概要: Gemini API のレート制限が厳しいため、Azure OpenAI に切り替えて全件処理を実現
- 詳細:
  - **変更内容**:
    - `@ai-sdk/azure` パッケージをインストール
    - `scripts/generate-summaries.ts` を Azure OpenAI 用に書き換え
      - `google()` → `azure()` に変更
      - 環境変数を `AZURE_OPENAI_*` に変更
      - レート制限対策の待機時間を削除（不要になった）
      - フィードあたりの記事数を3件→10件に戻す（全件処理）
    - `.env.example` を更新
      - `AZURE_OPENAI_API_KEY`
      - `AZURE_OPENAI_ENDPOINT`
      - `AZURE_OPENAI_DEPLOYMENT_NAME`
    - `.github/workflows/update-feeds.yml` を更新
      - 環境変数を Azure OpenAI 用に変更
  - **Azure OpenAI の利点**:
    - より高いレート制限（TPM: 数万トークン/分）
    - 安定したパフォーマンス
    - エンタープライズ向けサポート
- 次のアクション: `.env.local` に Azure OpenAI の認証情報を設定し、ローカルでテスト実行

## 22:56 Azure OpenAI 環境変数修正とテスト成功

- 概要: `@ai-sdk/azure` の正しい環境変数名を特定し、全90件の記事処理に成功
- 詳細:
  - **環境変数名の修正**:
    - `AZURE_OPENAI_API_KEY` → `AZURE_API_KEY` に変更
    - `AZURE_OPENAI_ENDPOINT` → `AZURE_RESOURCE_NAME` に変更（リソース名のみを指定）
    - `AZURE_OPENAI_DEPLOYMENT_NAME` はそのまま使用
  - **エラー原因の特定**:
    - `@ai-sdk/azure` は `AZURE_API_KEY` と `AZURE_RESOURCE_NAME` を期待
    - エンドポイント URL ではなく、リソース名のみを環境変数に設定
  - **テスト実行結果**:
    - 成功: 90件
    - エラー: 0件
    - 重複排除: 90件 → 90件（重複なし）
    - 処理時間: 約2分
  - **更新ファイル**:
    - `.env.example` を正しい環境変数名に修正
    - `scripts/generate-summaries.ts` の環境変数チェックを修正
    - `.github/workflows/update-feeds.yml` の環境変数を修正
- 次のアクション: GitHub Secrets に環境変数を設定し、GitHub Actions でテスト実行

## 23:07 記事履歴保持機能の実装

- 概要: 毎時更新で古い記事が消えないよう、履歴を保持する機能を実装
- 詳細:
  - **実装内容**:
    - 既存の `data/news.json` を読み込み、新規記事とマージ
    - ID ベースで重複排除（同じ記事は上書き）
    - 日時降順でソート（新しい記事が上位）
    - 最新1000件のみ保持（古い記事は自動削除）
  - **動作確認**:
    - 既存記事: 90件
    - 新規取得: 90件
    - マージ後: 180件
    - 重複排除後: 91件（新しい記事が1件追加された）
    - 最終出力: 91件
  - **メリット**:
    - 過去の記事も閲覧可能
    - 更新されても記事が消えない
    - 1000件制限でファイルサイズを管理
- 次のアクション: GitHub にプッシュして、GitHub Actions で自動更新をテスト

## 00:30 GitHub Actions 権限エラー修正とワークフロー成功

- 概要: GitHub Actions の権限エラーを修正し、自動更新ワークフローが完全に動作することを確認
- 詳細:
  - **エラー内容**:
    - Git push 時に `Permission denied` エラー（403）
    - GitHub Actions に書き込み権限がなかった
  - **修正内容**:
    - `.github/workflows/update-feeds.yml` に `permissions: contents: write` を追加
    - GitHub Actions がリポジトリに自動コミット & プッシュできるよう設定
  - **動作確認**:
    - Azure OpenAI 接続成功
    - 80件の記事を正常に要約生成
    - 既存91件 + 新規80件 = 171件マージ → 94件に重複排除
    - メルカリエンジニアリングブログのみ 403 エラー（フィード側の問題）
    - Git commit & push が成功するはず（次回実行で確認）
  - **完了した機能**:
    - 毎時0分に RSS フィードを自動取得
    - Azure OpenAI で記事を要約
    - 重複排除と履歴保持（最大1000件）
    - 自動コミット & プッシュ
- 次のアクション: Step 6-2 Netlify 連携の設定（または Expo アプリのデプロイ）

## 10:13 AndroidアプリでのNews.json取得エラーを修正

- 概要: ネイティブアプリで news.json が取得できない問題を解決
- 詳細:
  - **問題**:
    - Androidアプリで `/data/news.json` を fetch しようとすると Not Found エラー
    - 相対パスはWebでは動くが、ネイティブアプリでは動かない
  - **修正内容**:
    - `app/(tabs)/index.tsx` に Platform.OS による分岐を追加
    - Web: 相対パス `/data/news.json` (Netlifyから配信)
    - Native: 絶対URL `https://recaplet.netlify.app/data/news.json`
    - `app.json` に `extra.apiUrl` を追加（環境変数で上書き可能）
  - **実装の工夫**:
    - `Constants.expoConfig?.extra?.apiUrl` でフォールバック可能に
    - 本番URLをハードコードしつつ、開発時は環境変数で変更可能
- 次のアクション: Android APK ビルドを実行して動作確認
